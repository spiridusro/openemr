...
final class Connector {
    ...
    private function createConnection() {
        ...
        $this->logger->debug("Connecting with " . ($GLOBALS["doctrine_connection_pooling"] ? "pooled" : "non-pooled") . " mode");
        $connection = array(
            'driver'   => "pdo_mysql",
            'host'     => $sqlconf["host"],
            'port'     => $sqlconf["port"],
        ...
        $this->logger->debug("Wiring up Doctrine entities");
        $this->logger->debug("Database ". ($GLOBALS["doctrine_dev_mode"] ? "is" : "isn't") . " in development mode");
        $configuration = Setup::createAnnotationMetadataConfiguration($entityPath, $GLOBALS["doctrine_dev_mode"], null, null, false);

        $this->logger->debug("Creating connection");
        $this->entityManager = EntityManager::create($connection, $configuration);

        $this->logger->debug("Wiring up SQL auditor to store audit entries in `log` table");
